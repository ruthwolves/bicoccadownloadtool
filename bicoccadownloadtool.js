...
const progress = require("ffmpeg-progressbar-cli");
...
const argv = yargs.options({
  ...
  .help('h')
  .alias('h', 'help')
  .example('node $0 -u CODICEPERSONA -v "https://elearning.unimib.it/mod/kalvidres/view.php?id=65435"\n', "Standard usage").argv;
...
function sanityChecks() {
  try {
    ...
    term.red('Necessiti di aria2c in questa cartella! (O aggiunto in $PATH).');
    process.exit(22);
  }
  try {
    ...
    term.red('Necessiti di ffmpeg in questa cartella! (O aggiunto in $PATH).');
    process.exit(23);
  }
  ...
  if (argv.videoUrls !== undefined && argv.videoUrlsFile !== undefined) {
    term.red("Devi scegliere: o li metti in un file o ne scrivi uno su riga di comando.\n");
    process.exit();
  }
...
async function downloadVideo(videoUrls, username, password, outputDirectory) {
  ...
  if (password === undefined) { // password not passed as argument
    ...
    if (argv.noKeyring === false) {
      try {
        await keytar.getPassword("CoccaDown", username).then(function (result) {
          password = result;
        });
        if (password === null) { // no previous password saved
          password = await promptQuestion("Password non salvata. Scrivila di seguito e non sarà necesseraio reinserirla :-)    ");
          await keytar.setPassword("CoccaDown", username, password);
        } else {
          console.log("Riutilizzo password salvata")
        }
      } catch (e) {
        console.log("Non trovo X11 installato e perciò la password non può essere salvata.")
        password = await promptQuestion("Nessun problema, inseriscila manualmente: ");
      }
    } else {
  ...
  await browser.waitForTarget(target => target.url().includes('elearning.unimib'));
  console.log('Loggati!');
  ...
  for (let videoUrl of videoUrls) {
    ...
    try {
      await page.waitForSelector('span[class="card-title course-fullname text-truncate"]')
    } catch (e) {
    ...
    try {
      const titolo_sel = await page.waitForSelector('a[title="Kaltura Video Resource"]');
      var titolo = await page.evaluate(titolo_sel => titolo_sel.textContent, titolo_sel);
    ...
    try {
      ...
      const elementHandle = await page.$('iframe[class="kaltura-player-iframe"]');
      const first_frame = await elementHandle.contentFrame();

      await first_frame.waitForSelector('iframe[class="mwEmbedKalturaIframe"]');
      const elementHandle2 = await first_frame.$('iframe[class="mwEmbedKalturaIframe"]');
    ...
    try {
      ...
      await second_frame.click('a[class="icon-play  comp largePlayBtn  largePlayBtnBorder"]');
      while (text4 == "") {
        await sleep(100)
      }
    } catch (e) {
    ...
    try {
      ...
      var ffmpegCmd = "aria2c --summary-interval=0 --console-log-level=error --download-result=hide  \"" + text4 + "\" -o index.m3u8 -d temp";
      var ffmpegOpts = {
        stdio: 'inherit'
      };
      ...
      ffmpegCmd = "aria2c --summary-interval=0 --console-log-level=error --download-result=hide -j 16 -x 16 -i temp/index.m3u8 -d temp ";
      var result = execSync(ffmpegCmd, ffmpegOpts);

      console.log("\nUnisco i frammenti in un unico file mp4")
      ffmpegCmd = "ffmpeg-bar -protocol_whitelist file,http,https,tcp,tls,crypto,data -allowed_extensions ALL -i temp/index.m3u8 -c copy -y videos/" + titolo + ".mp4";
      var result = execSync(ffmpegCmd, ffmpegOpts);
...
term.brightBlue(`Progetto basato su PoliDown di sup3rgiu github.com/sup3rgiu \n`);
sanityChecks();
